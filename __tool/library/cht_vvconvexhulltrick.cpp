...
template <typename T, typename TQuery, typename TElement>
class ConvecHullTrick {
 ...
 public:
  ConvexHullTrick(vector<TElement> vals) {
    ConvexHullTrick(false);
    vector<PII> lines;
    transform(vals.begin(), vals.end(), back_inserter(lines),
              [](TInput val) { return PII(val.a, val.b); });
  ...
  ConvecHullTrick(bool flagX, function<bool(T, T)> compFunc =
                                  [](T l, T r) { return l >= r; })
      : isMonotonicX(flagX), comp(compFunc) {
    lines.emplace_back(-1LL << 20, 1LL << 41);  // TODO
  }
  ...
  T query(TQuery input) { return get(input.val); }
};